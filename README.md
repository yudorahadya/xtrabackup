...
1. Buat Folder /var/log/xtrabackup/

mkdir /var/log/xtrabackup/

2. Buat Folder /var/lib/xtrabackup/
...
5. Membuat folder /opt/script

mkdir /opt/script/

6. Copy dan paste script lalu ditempatkan di folder /opt/script/

cp xtrabackupnewrev1.sh /opt/script/
...
7. Edit file xtrabackupnewrev1.sh ubah user dan password sesuai yang digunakan di mysql

USER_ARGS=" --user=user_mysql --password=password_mysql"

8. Untuk menjalankan fullbackup jalankan script dibawah ini :

/bin/sh /opt/script/xtrabackupnewrev1.sh

9. Kemudian lakukan proses incremental backup dengan menjalankan script dibawah ini :
...
extract file fullbackup dan incrementalbackup, last_incremental_number dan pindahkan ke folder /var/log/xtrabackup/
